...
\begin{tikzpicture}[
    ...
    vblock/.style={anchor=north, align=center}
]

\node[box, text width=2.25cm] (pvec) {Estimated\\p\_vector};
\node[box, text width=2.25cm, right=0.5cm of pvec] (premap) {Premap};
\node[box, text width=2.65cm, right=0.5cm of premap] (pretrans) {Pretransform};
\node[box, text width=3.0cm, right=0.75cm of pretrans] (posttrans) {Posttransform};
...
% verbatim-ish tables underneath
\node[vblock, text width=2.25cm, below=0.25cm of pvec] (pvec_tab) {
...
\node[vblock, text width=2.25cm, below=0.25cm of premap] (premap_tab) {
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily]
t      B v v_lMd
1 1.0010 1     1
2 1.0020 1     1
...
\node[vblock, text width=2.65cm, below=0.25cm of pretrans] (pretrans_tab) {
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily]
t   lR lM   v      B
1 even  T 1.5 1.0010
1  odd  F 0.5 1.0010
...
\node[vblock, text width=3cm, below=0.25cm of posttrans] (posttrans_tab) {
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily]
t   lR lM      v      B
...
% more arrows
\draw[->] (pvec_tab) -- (premap_tab);
\draw[->] (premap_tab.east) -- (premap_tab.east -| pretrans_tab.west);
\draw[->] (pretrans_tab) -- (posttrans_tab);
...
```
```{r AM_pp_gen_RUN, echo=FALSE, fig.height=3.5, fig.width=4} 
...
plot_history_effects(dat, pp_ddm_am)
compare(list(rdm=emc, ddm=emc_ddm_am))
```
```{r ddm_example_AM_RUN, echo=FALSE}
trend_AM_DDM <- make_trend(cov_names='error',
                           kernels='delta',
                           par_names='a',
                           bases='lin',
