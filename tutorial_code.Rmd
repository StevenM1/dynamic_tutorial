...
On a group-level, we find a small (not credible) positive `B.w` -- on average, in this dataset, thresholds appear to increase by \~0.07 over the course of 1000 trials. We can now generate posterior predictives, and while doing that, ask *EMC2* to return the mapped and transformed parameters, which will allow us to visualize the fitted thresholds over trials:
...
for(subject in 1:6) {
  plot_trend(attr(pp, 'trialwise_parameters'), emc=emc,
             par_name='B', subject=subject, filter = function(data) data$lR == 'odd', main=subject)
}
```
...
for(subject in 1:6) {
  plot_trend(attr(pp, 'trialwise_parameters'), emc=emc,
             par_name='B', subject=subject, filter = function(data) data$lR == 'odd',
...
plot_trend(p_vector1, emc=emc_incr, par_name='v',
           subject=1, filter = function(data) data$lM == TRUE, main='v correct accumulator', ylim=c(3.5,5))
plot_trend(p_vector1, emc=emc_incr, par_name='v',
           subject=1, filter = function(data) data$lM == FALSE, main='v incorrect accumulator', ylim=c(1, 2.5))
...
plot_trend(p_vector, emc=emc_multitrend, par_name='B',
           subject=1, filter = function(data) data$lR == 'odd', main='Thresholds',
           xlab='Trial number')
plot_trend(p_vector, emc=emc_multitrend, par_name='v',
           subject=1, filter = function(data) data$lM == TRUE, main='Drift rate (matching)',
...
SM_trend <- make_trend(cov_names=c('Stim1'),
                       ...
                       bases='lin',
                       phase = "premap",
                       at="lR")  # see advanced note for an explanation of at
...
The left panel of this plot shows the mean RT as a function of the three previous trials' stimuli (ooo = odd, odd, odd; ooe = odd, odd, even; etc. The right-most letter in the string indicates the most recent trial). The RT data show a cross-over pattern, such that if the previous three stimuli were odd, 'odd' responses are faster than 'even' responses; and vice versa. The model tends to capture this cross-over pattern fairly well, with some misfits mostly due to an apparent asymmetry in the cross-over in the data.
...
for(subject in 1:3) {
  plot_trend(attr(pp, 'trialwise_parameters'), emc=emc,
             par_name='B', subject=subject, filter= function(data) data$lR == 'odd', main=paste0(subject, ' odd'),
             xlim=c(1,20))
  plot_trend(attr(pp, 'trialwise_parameters'), emc=emc,
             par_name='B', subject=subject, filter= function(data) data$lR == 'even', main=paste0(subject, ' even'),
             xlim=c(1,20))
}
```
...
pp <- predict(emc, n_cores=20, conditional_on_data=FALSE)
...
Here, we implement this using a custom kernel.
...
writeLines(c(
  ...
  "    // Apply base to B to get the correct threshold for next trial (needed for PE calculation)",
...
#save(emc, file='./samples/ds1_FM_new.RData')
...
#debug(make_data)
pp <- predict(emc, n_cores=20, conditional_on_data=FALSE, n_post = 50, return_trialwise_parameters=TRUE)
