...
# remotes::install_github("ampl-psych/EMC2",dependencies=TRUE, Ncpus=8);.rs.restartR()
# END TMP
library(EMC2)
library(Hmisc)
...
```
```{r make_figure_for_paper_1, eval=FALSE, include=FALSE}
emc1 <- make_emc(dat, design=design_RDM_lin_B)
emc2 <- make_emc(dat, design=design_RDM_lin_B2)
emc3 <- make_emc(dat, design=design_RDM_lin_B3)

p_vector <- c('B'=1, 'v'=1, 'v_lMd'=1, 't0'=0.1, 'B.w'=1)

pdf(file='./figures_paper/linear_trends.pdf', width=6, height=2.5)
par(mfrow=c(1,3))
# Visualize trend
plot_trend(p_vector, emc=emc1,
           par_name='B', subject=1,
           # filter here is a function that selects only the rows in the dadm
           # corresponding to the 'odd' accumulator
           filter = function(data) data$lR == "odd",
           main='A. Threshold for odd')

# visualize trend
plot_trend(p_vector, emc=emc2,
           par_name='B', subject=1,
           filter=function(data) data$lR == 'odd', main='B. Threshold for odd')

# visualize trend
plot_trend(p_vector, emc=emc3,
           par_name='B', subject=1,
           filter=function(data) data$lR == 'odd', main='C. Threshold for odd')
dev.off()
```
...
dadm <- emc1[[1]]$data[[1]]
model <- emc1[[1]]$model()
pars_mapped <- EMC2:::map_p(EMC2:::add_constants(p_vector, attr(dadm, "constants")), dadm, model)
...
```{r linear_trend_fit_load, include=FALSE}
...
```{r make_figures_paper_sm, eval=FALSE, include=FALSE}
load('./ds1_SMB_pps.RData')
pdf(file='./figures_paper/sm_rdm.pdf', width=7, height=3.5)
plot_history_effects(dat, pp, n_hist=3)
dev.off()
```
...
```{r make_figures_paper_rm2, eval=FALSE, include=FALSE}
pdf(file='./figures_paper/rm_fit.pdf', width=7, height=3)
plot_history_effects(dat, pp, degree=2)
dev.off()
```
...
pp <- predict(emc, n_cores=40)
save(pp, file='./samples/ds1_AM_pps.RData')
```

```{r AM_pp_load, include=FALSE}
