...
print_par <- function(layout = "1panel") {
  switch(layout,
    ...
    "1x5"    = par(mar = c(2.5, 2.5, 1.2, 0.2), mgp = c(1.4, 0.4, 0), tcl = -0.25,
                   mfrow = c(1, 5),
                   cex = 0.49, cex.axis = 1, cex.lab = 1.143, cex.main = 1.143),
...
This process of combining *sampled* parameters into *estimated* parameters is what we call design *mapping* --- this will play an important role later when specifying trends. By default, `design()` prints the design matrices, but `mapped_pars()` provides some additional information:
...
plot(prior_RDM)
```
```{r baseline-rdm-priors-run, echo=FALSE, fig.width=5, fig.height=1.8, fig.cap=cap, dependson="baseline-rdm-design"}
...
check(emc_rdm)
```
```{r baseline-checks-run, echo=FALSE, fig.width=5.5, fig.height=2, fig.cap=cap, dependson="baseline-fit-run"}
...
```{r baseline-rdm-plotpars-run, fig.height=1.8, fig.width=5, echo=FALSE, fig.cap=cap, dependson="baseline-fit-run"}
...
plot_cdf(emc_rdm, pp_rdm, 
         ...
         factors=c("S"),
         # with the nopar=TRUE argument, we request EMC2 not to handle the layout, which we do here manually to plot the models side-by-side
         nopar=TRUE)
plot_cdf(emc_wdm, pp_wdm, factors=c("S"), nopar=TRUE)

```

```{r baseline-predict-pp-run, echo=FALSE, fig.height=2, fig.width=6, fig.cap=cap, dependson="baseline-fit-run"}
...
print_par("1x4")
plot_cdf(emc_rdm, pp_rdm, factors=c("S"), nopar=TRUE, xlim=c(0, 1.3))
plot_cdf(emc_wdm, pp_wdm, factors=c("S"), nopar=TRUE, xlim=c(0, 1.3))
```
...
```{r baseline-recovery-plot-run, echo=FALSE, fig.width=6, fig.height=2., fig.cap=cap, dependson="baseline-recovery-run"}
...
print_par("1x4")
recovery(emc_simulated_rdm, true_pars, selection="alpha", nopar=TRUE)
```
...
knitr::opts_chunk$set(fig.cap = cap_mapping)
```
```{tikz fig-mapping, echo=FALSE, fig.align='center', out.width='100%',         fig.ext=if(knitr::is_html_output()) 'svg' else 'pdf',         engine.opts=list(convert='pdf2svg')}
...
```{r lin-plot-run, echo=FALSE, fig.width=2.0, fig.height=2.0, dependson="lin-design"}
emc_lin <- make_emc(dat, design=RDM_lin_B)
...
```{r lin-phase-run, echo=FALSE, fig.height=2.0, fig.width=4, dependson="lin-design"}
# Sample on log scale, but apply trend posttransform
lin_trend2 <- make_trend(cov_names="trials2",  kernels="lin_incr",
                         par_names="B", bases="lin",
                         # We only change phase to posttransform:
...
```{r lin-priors-run, echo=FALSE, fig.width=6, fig.height=1.8, fig.cap=cap, dependson="lin-phase-run"}
...
```{r lin-check-run, echo=FALSE, fig.width=5.5, fig.height=2, fig.cap=cap, dependson="lin-fit-run"}
invisible(capture.output({ckc <- check(emc, plot_worst=FALSE)}))
subn <- which.max(lapply(ckc$alpha, function(x) max(x[1,])))
cap <- paste0("Trace plots of the sampled parameters with highest Rhat values (i.e., worst convergence) at the group-level mean (left, mu), the group-level variance (middle, sigma2), and the subject-level (alpha; in this case, subject number ", subn, "). ESS = effective sample size.")

print_par("1x3")
check(emc, nopar=TRUE)
```
...
```{r lin-posterior-run, echo=FALSE, fig.width=5.5, fig.height=1.8, fig.cap=cap, dependson="lin-fit-run"}
...
```{r lin-pp-run, echo=FALSE, fig.height=2.5, fig.width=5, fig.cap=cap, dependson="lin-fit-run"}
...
plot_trend(attr(pp, "trialwise_parameters"), emc=emc,
           par_name="B",
           filter=function(data) data$lR=="odd", nopar=TRUE)
```
...
```{r exp-design-run, echo=FALSE, fig.width=4, fig.height=1.8, fig.cap=cap, dependson="load-data-trials"}
...
plot_trend(p_vector3, emc=emc_incr, par_name="v",
           ...
           main="v mismatching", ylim=c(1, 2.5))
```
```{r exp-direction-run, echo=FALSE, fig.height=3, fig.width=5, fig.cap=cap}
...
```{r multitrend-run, echo=FALSE, fig.height=1.8, fig.width=5, fig.cap=cap, dependson="load-data-trials"}
# simulate block number (assume 3 blocks)
dat$block <- as.factor(as.numeric(cut(dat$trials, breaks=3)))
for(subject in unique(dat$subjects)) {
  dat[dat$subjects==subject,"trial_in_block"] <- ave(
...
```{r poly-plot-run, echo=FALSE, fig.height=1.8, fig.width=5.25, fig.cap=cap, dependson="load-data-trials"}
...
```{r sm-check-run, echo=FALSE, fig.width=5.5, fig.height=2, fig.cap=cap, dependson="sm-fit-run"}
invisible(capture.output({ckc <- check(emc, plot_worst=FALSE)}))
subn <- which.max(lapply(ckc$alpha, function(x) max(x[1,])))
cap <- paste0("Trace plots of the sampled parameters with highest Rhat values (i.e., worst convergence) at the group-level mean (left, mu), the group-level variance (middle, sigma2), and the subject-level (alpha; in this case, subject number ", subn, "). ESS = effective sample size.")
print_par("1x3")
...
```{r sm-posterior-run, echo=FALSE, fig.height=3, fig.width=5, fig.cap=cap, dependson="sm-fit-run"}
...
```{r sm-pp-run, echo=FALSE, fig.height=2.25, fig.width=4, fig.cap=cap, dependson="sm-fit-run"}
...
par(cex=.7)
plot_history_effects(dat, pp, n_hist=3, nopar=TRUE)
```
...
```{r sm-trialwise-run, echo=FALSE, fig.height=3, fig.width=5, fig.cap=cap, dependson="sm-pp-run"}
...
for(subject in 1:3) {
  plot_trend(attr(pp, "trialwise_parameters"), emc=emc,
             par_name="B", subject=subject, 
             filter=function(data) data$lR=="odd", 
             main=paste0(subject, " odd"), xlim=c(1,20), nopar=TRUE)
  plot_trend(attr(pp, "trialwise_parameters"), emc=emc,
             par_name="B", subject=subject, 
             filter=function(data) data$lR=="even", 
             main=paste0(subject, " even"), xlim=c(1,20), nopar=TRUE)
}
```
...
```{r sm-qvalues-run, echo=FALSE, fig.height=3, fig.width=5, fig.cap=cap, dependson="sm-pp-run"}
...
```
```{r rm-simulate-run, echo=FALSE, fig.height=2.25, fig.width=4, fig.cap=cap, dependson="rm-design"}
...
par(mar=c(3,3,2.5,0.5))
par(cex=.7)
plot_history_effects(dat=simDat, degree=2, nopar=TRUE)
```
...
```
```{r rm-fit-run, echo=FALSE, fig.height=2.25, fig.width=4, fig.cap=cap, dependson="rm-design"}
...
plot_history_effects(dat, pp, degree=2, nopar=TRUE)
```
...
compare(sList=list(rdm=emc,
                   ddm=emc_ddm))
```
```{r sm-ddm-run, echo=FALSE, fig.height=2.25, fig.width=4, fig.cap=cap, dependson="sm-fit-run"}
...
# check quality of fit
plot_history_effects(dat, pp_ddm, nopar=TRUE)
...
```
```{r am-pp-run, echo=FALSE, fig.height=2.25, fig.width=2.25, fig.cap=cap, dependson="am-fit-run"} 
...
```
```{r am-ddm-run, echo=FALSE, fig.cap=cap, fig.height=2.25, fig.width=2.25, dependson="am-fit-run"}
...
```
```{r fm-pp-run, fig.height=2.25, fig.width=2.25, echo=FALSE, dependson="fm-custom-run", fig.cap=cap}
