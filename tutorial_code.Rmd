...
knitr::opts_chunk$set(
  ...
  collapse    = FALSE,        # merges code + output into one block
  results     = 'markup',
  fig.path    = "figs/",
...
print_par <- function(layout = "1panel") {
  switch(layout,
    "1panel" = par(mar = c(3, 3, 1.5, 0.5),     mgp = c(1.8, 0.5, 0), tcl = -0.3,
                   cex = 0.8, cex.axis = 0.8, cex.lab = 0.9, cex.main = 0.9),
    ...
    "1x4"    = par(mar = c(3, 2.5, 1.5, 0.5),   mgp = c(1.5, 0.4, 0), tcl = -0.25,
                   mfrow = c(1, 4),
                   cex = 0.75, cex.axis = 0.75, cex.lab = 0.85, cex.main = 0.85),
    "1x5"    = par(mar = c(2.5, 2.5, 1.2, 0.2), mgp = c(1.4, 0.4, 0), tcl = -0.25,
                   mfrow = c(1, 5),
                   cex = 0.7, cex.axis = 0.7, cex.lab = 0.8, cex.main = 0.8),
...
Note that the design matrix for drift rates `v` is similar to the DDM case, but the rows now correspond to the matching and mismatching accumulator rather than the two stimulus levels. Inspecting `mapped_pars()`:
...
```{r baseline-rdm-plot_pars-code, eval=FALSE}
plot_pars(emc_rdm)
```

```{r baseline-rdm-plot_pars-run, fig.height=1.8, fig.width=5, echo=FALSE, fig.cap=cap, dependson="baseline-fit-run"}
...
```{r baseline-predict-pp-run, echo=FALSE, fig.height=1.8, fig.width=5, fig.cap=cap, dependson="baseline-fit-run"}
...
```{r lin-mapped_pars, dependson="lin-design"}
...
```{r lin-pp-run, echo=FALSE, fig.height=3, fig.width=5, fig.cap=cap, dependson="lin-fit-run"}
